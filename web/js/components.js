...
const Components = {
     ...
     * @param {Object} schedule - Schedule data with l1_users array
     * @param {Object} onCallResult - Current on-call result
     * @param {string} teamId - Team ID
     */
    onCallWidget: (schedule, onCallResult, teamId) => {
        const l1Users = onCallResult?.l1_users || [];
        const l2User = onCallResult?.l2_user;
        const override = onCallResult?.override;
        const l1Names = l1Users.map(u => u.name).join(', ');
        let untilText = 'N/A';
        if (onCallResult?.l1_until) {
            const dateStr = new Date(onCallResult.l1_until).toLocaleString(undefined, {
                month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit'
            });
            untilText = `until ${dateStr}`;
        } else if (l1Users.length > 0) {
            untilText = 'forever';
        }

        return `
            <div class="on-call-widget" data-team-id="${teamId}" data-schedule-id="${schedule?.id || ''}">
                ...
                <div class="on-call-user">
                    ...
                    <div class="on-call-user-info">
                        ...
                        ` : ''}
                        <span class="on-call-until">${untilText}</span>
                    </div>
                </div>
                ${schedule?.l2_enabled && l2User ? `
                    <div class="on-call-l2">
                        <span class="on-call-l2-label">L2 Backup:</span>
                        <span class="on-call-l2-name">${escapeHtml(l2User.name)}</span>
                    </div>
                ` : ''}
                <div class="on-call-actions">
                    ${Permissions.can('manage_schedule', { teamId: teamId }) ? `
                    <button class="btn btn-sm btn-secondary edit-schedule-btn" data-team-id="${teamId}">
                        <i data-lucide="settings"></i>
                        Configure
                    </button>
                    ` : ''}
                    <button class="btn btn-sm btn-secondary view-schedule-btn" data-team-id="${teamId}">
                        <i data-lucide="calendar-days"></i>
                        View Schedule
                    </button>
                    ${Permissions.can('create_override', { teamId: teamId }) ? `
                    <button class="btn btn-sm btn-primary create-override-btn" data-team-id="${teamId}">
                        <i data-lucide="user-plus"></i>
                        Override
                    </button>
                    ` : ''}
     ...
     */
    onCallOverviewRow: (schedule, onCallResult, team) => {
        ...
        const hasOnCall = l1Users.length > 0;
        ...
        return `
            <div class="oncall-row" data-team-id="${escapeHtml(teamId)}" data-schedule-id="${schedule?.id || ''}">
                ...
                <div class="oncall-cell" data-label="Status">
                    <span class="badge badge-status oncall-status-badge ${statusClass}">
                        ${statusLabel}
                        ${override && canDeleteOverride ? `<button class="delete-override-btn" data-schedule-id="${schedule?.id}" data-override-id="${override.id}" title="Remove override"><i data-lucide="x"></i></button>` : ''}
                    </span>
                </div>
                <div class="oncall-cell oncall-cell-actions">
                    ${canOverride && schedule ? `
                    <button class="btn btn-sm btn-secondary create-override-btn" data-team-id="${teamId}">
                        <i data-lucide="user-plus"></i>
                        Override
                    </button>
                    ` : ''}
    ...
    /**
     * Render schedule configuration modal
     * @param {Object} schedule - Schedule data (null for new)
     * @param {Array} teamMembers - Team members for user selection
     * @param {string} teamId - Team ID
     */
    scheduleConfigModal: (schedule, teamMembers, teamId) => {
        const l1Groups = schedule?.l1_groups || [];
        const l2Users = schedule?.l2_users || [];

        // L2 split (still flat list)
        const l2SelectedIds = new Set(l2Users.map(u => u.id));
        const l2Available = teamMembers.filter(u => !l2SelectedIds.has(u.id));

        const renderUserItem = (user, showGrip = true) => `
            <div class="rotation-user" data-user-id="${user.id}">
                ${showGrip ? '<i data-lucide="grip-vertical" class="drag-handle"></i>' : ''}
                <div class="rotation-user-avatar">${escapeHtml(user.name.charAt(0).toUpperCase())}</div>
                <span class="rotation-user-name">${escapeHtml(user.name)}</span>
            </div>
        `;

        // Render a single user chip inside a group
        const renderUserChip = (user) => `
            <span class="user-chip" data-user-id="${user.id}">
                ${escapeHtml(user.name)}
                <button type="button" class="chip-remove" aria-label="Remove">×</button>
            </span>
        `;

        // Render the "Add user" select for a group
        const renderAddUserSelect = () => `
            <select class="form-select group-add-user">
                <option value="">+ Add user</option>
                ${teamMembers.map(u => `<option value="${escapeHtml(u.id)}">${escapeHtml(u.name)}</option>`).join('')}
            </select>
        `;

        // Render a group row
        const renderGroupRow = (group, index) => `
            <div class="group-row" data-group-index="${index}">
                <i data-lucide="grip-vertical" class="group-drag-handle"></i>
                <span class="group-label">Group ${index + 1}</span>
                <div class="group-chips">
                    ${(group || []).map(u => renderUserChip(u)).join('')}
                </div>
                ${renderAddUserSelect()}
        ...
        return `
            <form id="schedule-form" data-team-id="${teamId}">
                <div class="schedule-config-grid">
                        ...
                        <div class="form-section">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" id="slack-usergroup-id" name="slack_usergroup_id" 
                                       class="form-input"
                                       value="${schedule?.slack_usergroup_id || ''}"
                            ...
                            <div class="form-row">
                                <div class="form-group">
                                    ...
                                    <select id="l1-rotation-type" name="l1_rotation_type" class="form-select">
                                        <option value="daily" ${schedule?.l1_rotation_type === 'daily' ? 'selected' : ''}>Daily</option>
                                        <option value="weekly" ${schedule?.l1_rotation_type !== 'daily' ? 'selected' : ''}>Weekly</option>
                                    </select>
                                </div>
                                <div class="form-group">
                                    <label for="l1-handoff-time">Handoff</label>
                                    <input type="time" id="l1-handoff-time" name="l1_handoff_time" 
                                           value="${schedule?.l1_handoff_time || '11:00'}" class="form-input">
                                </div>
                            </div>
                            <div class="form-group l1-weekly-only" style="${schedule?.l1_rotation_type === 'daily' ? 'display:none' : ''}">
                                <label for="l1-handoff-day">Handoff Day</label>
                                <select id="l1-handoff-day" name="l1_handoff_day" class="form-select">
                                    <option value="1" ${schedule?.l1_handoff_day === 1 ? 'selected' : ''}>Monday</option>
                                    <option value="2" ${schedule?.l1_handoff_day === 2 ? 'selected' : ''}>Tuesday</option>
                                    <option value="3" ${schedule?.l1_handoff_day === 3 ? 'selected' : ''}>Wednesday</option>
                                    <option value="4" ${schedule?.l1_handoff_day === 4 ? 'selected' : ''}>Thursday</option>
                                    <option value="5" ${schedule?.l1_handoff_day === 5 ? 'selected' : ''}>Friday</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="form-section">
                            <h4 class="form-section-title">
                                <label class="toggle-label">
                                    <input type="checkbox" id="l2-enabled" name="l2_enabled" ${schedule?.l2_enabled ? 'checked' : ''}>
                                    <i data-lucide="shield"></i>
                                    L2 Escalation
                                </label>
                            </h4>
                            <div class="l2-config" style="${!schedule?.l2_enabled ? 'display:none' : ''}">
                                <div class="form-group">
                                    ...
                                    <input type="number" id="l2-escalation-timeout" name="l2_escalation_timeout" 
                                           value="${schedule?.l2_escalation_timeout_min || 5}" min="1" max="60" class="form-input">
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="schedule-users">
                        ...
                        <div class="rotation-panel l2-users-panel" style="${!schedule?.l2_enabled ? 'display:none' : ''}">
                            <h4 class="rotation-panel-title">
                                <i data-lucide="shield"></i>
                                L2 Backup Rotation
                            </h4>
                ...
                <div class="team-modal-section" style="margin-top: 24px; padding-top: 16px; border-top: 1px solid var(--border-color);">
                    ...
                    <p style="font-size: 0.85rem; color: var(--text-muted); margin-bottom: 12px;">
                        Deleting this schedule will remove all rotations and overrides. This cannot be undone.
                    </p>
                    <button type="button" class="btn btn-sm btn-danger delete-schedule-btn" data-team-id="${teamId}">
                        <i data-lucide="trash-2" style="width:14px;height:14px;"></i>
                        Delete Schedule
                    </button>
                </div>
    ...
    /**
     * Render override list block
     * @param {Array} overrides - Existing overrides
     * @param {string} scheduleId - Schedule ID for delete operations
     */
    overridesList: (overrides = [], scheduleId = '') => {
                ...
                ${overrides.map(o => `
                    <div class="override-item">
                        <div class="override-item-user">
                            <div class="override-item-avatar">${escapeHtml((o.user?.name || 'User').charAt(0).toUpperCase())}</div>
                            <span>${escapeHtml(o.user?.name || 'Unknown User')}</span>
                        </div>
    ...
    /**
     * Render override management modal
     * @param {Array} teamMembers - Team members for user selection
     * @param {string} teamId - Team ID
     * @param {Array} existingOverrides - Existing overrides
     * @param {string} scheduleId - Schedule ID for delete operations
     */
    overrideModal: (teamMembers, teamId, existingOverrides = [], scheduleId = '') => {
        const now = new Date();
        now.setSeconds(0, 0);
        const pad = (n) => String(n).padStart(2, '0');
        const nowLocal = `${now.getFullYear()}-${pad(now.getMonth()+1)}-${pad(now.getDate())}T${pad(now.getHours())}:${pad(now.getMinutes())}`;
        const tomorrowDate = new Date(now.getTime() + 24*60*60*1000);
        const tomorrowLocal = `${tomorrowDate.getFullYear()}-${pad(tomorrowDate.getMonth()+1)}-${pad(tomorrowDate.getDate())}T${pad(tomorrowDate.getHours())}:${pad(tomorrowDate.getMinutes())}`;

        const overridesList = Components.overridesList(existingOverrides, scheduleId);
        ...
        return `
            <div class="override-modal-content">
                ...
                <div class="override-form-section">
                    ...
                    <form id="override-form" data-team-id="${teamId}">
                        <div class="form-row">
                            <div class="form-group">
    ...
    /**
     * Render weekly calendar grid for schedule visualization
     * @param {Array} entries - Rendered schedule entries
     * @param {Date} weekStart - Start of the week to display
     */
    weeklyCalendar: (entries, weekStart) => {
        const days = [];
        for (let i = 0; i < 7; i++) {
        ...
        return `
            <div class="schedule-calendar">
                <div class="calendar-header">
                    <button class="btn btn-sm btn-secondary calendar-prev" title="Previous Week">
                        <i data-lucide="chevron-left"></i>
                    </button>
                    <span class="calendar-title">${weekStart.toLocaleDateString(undefined, { month: 'long', year: 'numeric' })}</span>
                    <button class="btn btn-sm btn-secondary calendar-next" title="Next Week">
                            ...
                            <div class="calendar-day ${isToday ? 'today' : ''}">
                                ...
                                <div class="calendar-day-content">
                                    ${dayEntries.length > 0 ? dayEntries.map(e => {
                                        const names = (e.user_names || []).join(', ');
                                        const firstNames = (e.user_names || []).map(n => n.split(' ')[0]).join(', ');
                                        return `
                                        <div class="calendar-entry layer-${e.layer}" title="${escapeHtml(names)}">
                                            ${escapeHtml(firstNames)}
                                        </div>
    ...
    /**
     * Render monthly schedule calendar as a list view
     * @param {Array} entries - Rendered schedule entries from API
     * @param {Date} startDate - Start date
     * @param {string} timezone - Schedule timezone
     */
    monthlyScheduleCalendar: (entries, startDate, timezone = 'UTC') => {
        ...
        // Group entries by date (use start_time's local date in the TARGET timezone)
        const entriesByDate = new Map();
        entries.forEach(entry => {
            // Parse start time
            const start = new Date(entry.start_time);
            // Construct key using the target timezone to ensure correct grouping
            const dateKey = start.toLocaleDateString('en-US', { timeZone: timezone });
        ...
        // Helper to check the calendar date string in the target timezone
        const getDateKey = (date) => date.toLocaleDateString('en-US', { timeZone: timezone });

        for (let i = 0; i < 30; i++) {
            days.push(new Date(current));

            // Robustly advance by 1 CALENDAR DAY in the TARGET timezone
            const currentKey = getDateKey(current);

            // Add ~24h (use UTC math to avoid browser local DST issues)
            let next = new Date(current.getTime() + 24 * 60 * 60 * 1000);

            // Check for drift (e.g., 25h day causing us to land on the same day)
            // If "next" still maps to the same date string, add extra time
            if (getDateKey(next) === currentKey) {
                next = new Date(next.getTime() + 12 * 60 * 60 * 1000); // Add 12h more to be safe
            }

            // Check for overshoot (e.g., 23h day causing us to skip?) 
            // 24h on a 23h day -> +1h into next day. Correct.
            // 24h on a 25h day -> -1h (same day). Catch above.
        ...
        const formatDate = (date) => {
            ...
            const todayKey = today.toLocaleDateString('en-US', { timeZone: timezone });
            const dateKey = date.toLocaleDateString('en-US', { timeZone: timezone });
        ...
        const getLayerClass = (layer) => {
            if (layer === 'l1') return 'layer-l1';
            if (layer === 'l2') return 'layer-l2';
            if (layer === 'override') return 'layer-override';
            return '';
        };
        ...
        const getLayerLabel = (layer) => {
            ...
            if (layer === 'override') return 'Override';
            return '';
        };

        // Format time for shifts - always show handoff time
        const formatShiftTime = (entry, tz) => {
            const start = new Date(entry.start_time);
            const end = new Date(entry.end_time);

            // function to formats date in target timezone
            const getDateStr = (d) => d.toLocaleDateString('en-US', { timeZone: tz });

            // Check if start and end are on different days in the VIEW timezone
            const sameDay = getDateStr(start) === getDateStr(end);

            // Format times in short format using the schedule timezone
            ...
            if (sameDay) {
                ...
                // End is on next day - show time or indicate it's full day
                const durationHours = (end - start) / (1000 * 60 * 60);
                if (durationHours >= 23.5) {
                    // ~24h segment - just show start time with indicator
        ...
        return `
            <div class="monthly-calendar">
                ...
                <div class="calendar-list">
                    ${days.map(day => {
            // Use the same timezone-aware key format for lookup
            const dayKey = day.toLocaleDateString('en-US', { timeZone: timezone });
            ...
            return `
                ...
                // Only show time if there are multiple entries for the day (handoff happens)
                const showTime = dayEntries.length > 1;
                const timeStr = showTime ? formatShiftTime(entry, timezone) : '';
                const userNames = (entry.user_names || []).join(', ') || 'Unknown';
                const firstUserId = (entry.user_ids && entry.user_ids[0]) || '';
                return `
                                        <div class="calendar-entry ${getLayerClass(entry.layer)}"
                                            ${entry.override_id ? `
                                                data-override-id="${entry.override_id}"
                                                data-schedule-id="${entry.schedule_id}"
                                                data-user-id="${firstUserId}"
                                                data-start-time="${entry.override_start}"
                                                data-end-time="${entry.override_end}"
                                                data-reason="${escapeAttr(entry.reason || '')}"
                                            ` : ''}>
                                            <span class="entry-layer">${getLayerLabel(entry.layer)}</span>
