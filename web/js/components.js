...
const Components = {
    ...
    /**
     * The two time facts about an assignment, kept apart (see §13 of the
     * schedule design): when the shift began by the rotation grid, and when
     * this particular assignment took effect. They coincide most of the time,
     * which is exactly why conflating them survives so long unnoticed.
     */
    assignmentTimes: (layer) => {
     ...
     */
    onCallOverviewRow: (onCall, team, ctx = {}) => {
        ...
        const namesRaw = Components.onCallNames(l1, names);
     ...
     */
    scheduleWarnings: (warnings, extra = {}) => {
        const banners = [];
     ...
     * @param {string} teamId
     */
    scheduleConfigModal: (state, teamId) => {
     ...
     */
    overridesList: (overrides = [], scheduleId = '', names = new Map()) => {
    ...
    overrideModal: (state, teamId) => {
        ...
        return `
            <div class="override-modal-content">
                ${Components.overridesList(state.overrides, state.scheduleId, state.names)}
     ...
     * @param {Map} names - id -> display name
     */
    monthlyScheduleCalendar: (render, startDate, timezone = 'UTC', names = new Map()) => {
        const entries = render?.entries || [];
        const banners = Components.scheduleWarnings(render?.warnings, {
     ...
     */
    schedulePreview: (preview, names = new Map(), timezone = 'UTC') => {
        const before = Components.onCallNames(preview.on_call_before?.l1, names) || 'nobody';
        const after = Components.onCallNames(preview.on_call_after?.l1, names) || 'nobody';
        ...
        return `
            <div class="schedule-preview">
                ...
                ${Components.scheduleWarnings(preview.warnings)}
