...
func TestOutboxStore(t *testing.T) {
	...
	t.Run("update event status", func(t *testing.T) {
		...
		event.Status = model.OutboxEventStatusCompleted
		if err := s.UpdateOutboxEvent(event); err != nil {
			t.Fatalf("UpdateOutboxEvent failed: %v", err)
		}

		fetched, _ := s.GetOutboxEventByID(event.ID)
		if fetched.Status != model.OutboxEventStatusCompleted {
			t.Errorf("Expected completed, got %s", fetched.Status)
		}
	})

	t.Run("create and get delivery", func(t *testing.T) {
		s := setupTestDB(t)
		...
		// Create integration for FK
		cfg := json.RawMessage(`{"url":"https://example.com","secret":"s"}`)
		...
		if err := s.CreateOutboxDelivery(delivery); err != nil {
			t.Fatalf("CreateOutboxDelivery failed: %v", err)
		}

		fetched, err := s.GetOutboxDelivery(event.ID, integration.ID)
		if err != nil {
			t.Fatalf("GetOutboxDelivery failed: %v", err)
		}
		if fetched.Status != model.OutboxDeliveryPending {
			t.Errorf("Expected pending, got %s", fetched.Status)
		}
	})

	t.Run("duplicate delivery rejected", func(t *testing.T) {
		s := setupTestDB(t)
		...
		d1 := &model.OutboxDelivery{EventID: event.ID, IntegrationID: integration.ID}
		if err := s.CreateOutboxDelivery(d1); err != nil {
			t.Fatalf("First delivery failed: %v", err)
		}

		d2 := &model.OutboxDelivery{EventID: event.ID, IntegrationID: integration.ID}
		if err := s.CreateOutboxDelivery(d2); err == nil {
			t.Error("Expected error for duplicate delivery")
		}
	})

	t.Run("get deliveries by event", func(t *testing.T) {
		s := setupTestDB(t)
		...
		deliveries, err := s.GetDeliveriesByEventID(event.ID)
		if err != nil {
			t.Fatalf("GetDeliveriesByEventID failed: %v", err)
		}
		if len(deliveries) != 2 {
	...
	t.Run("get deliveries by integration", func(t *testing.T) {
		s := setupTestDB(t)
		...
		deliveries, total, err := s.GetDeliveriesByIntegrationID(integration.ID, 2, 0)
		if err != nil {
			t.Fatalf("GetDeliveriesByIntegrationID failed: %v", err)
		}
		if total != 3 {
			t.Errorf("Expected total=3, got %d", total)
		}
		if len(deliveries) != 2 {
			t.Errorf("Expected 2 deliveries (limit), got %d", len(deliveries))
		}
	})
	...
	t.Run("update delivery", func(t *testing.T) {
		...
		s.CreateOutboxDelivery(delivery)

		httpStatus := 200
		delivery.Status = model.OutboxDeliverySent
		delivery.Attempts = 1
		delivery.LastHTTPStatus = &httpStatus
		if err := s.UpdateOutboxDelivery(delivery); err != nil {
			t.Fatalf("UpdateOutboxDelivery failed: %v", err)
		}

		fetched, _ := s.GetOutboxDelivery(event.ID, integration.ID)
		if fetched.Status != model.OutboxDeliverySent {
			t.Errorf("Expected sent, got %s", fetched.Status)
		}
		if fetched.Attempts != 1 {
			t.Errorf("Expected 1 attempt, got %d", fetched.Attempts)
		}
		if fetched.LastHTTPStatus == nil || *fetched.LastHTTPStatus != 200 {
			t.Errorf("Expected HTTP 200, got %v", fetched.LastHTTPStatus)
		}
	})
}
